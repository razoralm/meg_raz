...
TFCEinput_stc = path + 'target/data_for_TFCE/labeled/{0}_avg{1}_{2}_integ5_{3}-{4}.stc' 
...
output_meshtime_stc = path + "scripts/raz/TFCE_analyses/meshtime_results/3dTFCE_frame_avg{0}_integ5_e{1}h{2}n{3}-{4}.stc"
...
#output_explore = path + "/scripts/raz/TFCE_analyses/explore_results/3dEXPLORE_normalized_{0}_integ5_{1}_e{2}-{3}_h{4}-{5}_step{6}.txt"
output_explore = path + "/scripts/raz/TFCE_analyses/explore_results/3dEXPLORE_frame_avg{0}_integ5_e{1}-{2}_h{3}-{4}_step{5}.txt"
...
## set output stc names (for fixed params in mesh-time mode) 
                                ...
                                #3dTFCE_144_217ms_integ5_presylvian_e1h2n100-lh
                                #type_{time_interval}_integ_{label}_param-e{e}param-h{h}n_perm{n_perm}-{hemisphere}.stc
output_lh = output_meshtime_stc.format(time_lbl, e, h, n_perm, "lh")	
output_rh = output_meshtime_stc.format(time_lbl, e, h, n_perm, "rh")	
...
# output explore example:
						 #"3dEXPLORE_144_217ms_integ5_inf_operculum_e0-2_h0-2_step0.1_n100.txt"
    					 #"3dEXPLORE_{time_interval}_integ5_{label}_e{0}-{2}_h{0}-{2}_step{0.1}_n{}.txt"	
with open(output_explore.format(time_lbl, e, e_max, h, h_max, step, n_perm), 'w+') as outfile:
