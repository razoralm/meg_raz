...
############# mah 
	...
	#S12_sample = np.append(w1, np.append(d1 , np.append(w2, d2, axis = 0), axis = 0), axis = 0)
	_S1_sample = np.append(w1, d1, axis = 0)
	...
	for i, ind in enumerate(lbl_inds):
		...
		S1_cov = np.cov(_S1_sample[:,int(ind),:], rowvar=False)
		print('\n cov_',  ind, ' = \n', S1_cov)
	
		S1I = np.linalg.inv(S1_cov)
		print('\n inv_cov_',  ind, ' = \n', S1I)

		mah_S1_W2[i] = spatial.distance.mahalanobis(W2[int(ind),:], S1[int(ind),:], S1I)
		
		mah_S1_D2[i] = spatial.distance.mahalanobis(D2[int(ind),:], S1[int(ind),:], S1I)
...
time_lbls = ["185_195ms"]
time_intervals = np.array([[185, 200]])
